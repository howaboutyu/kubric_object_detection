...
SYNTHETIC_DATA_OUTPUT := kubric_synthetic_data_output
...
run-synthetic:
    ...
    --output-path $(SYNTHETIC_DATA_OUTPUT) \
    --num-generation 2000

create-tfrecord:
	@docker run -it \
    -v `pwd`:/kubric \
    $(KUBRIC_IMAGE) python /kubric/syndata_generator/tfrecord_creator.py \
    --input-dir $(SYNTHETIC_DATA_OUTPUT) \
    --output-folder tfrecord_output
