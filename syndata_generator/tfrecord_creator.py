...
def read_metadata(json_path):
    ...
    for instance in instances:
        ...
        if 'asset_id' not in instance: continue
...
def frame_to_example(
    ...
    example = tf.train.Example(features=tf.train.Features(feature={
      ...
      'image/key/sha256': dataset_util.bytes_feature(image_key.encode('utf8')),
...
def kubric_session_to_tf_example(
    kubric_session_dir,
    tfrecord_output_dir 
):
    ...
    output_file = os.path.join(tfrecord_output_dir, f'kubric_session_{session_id}.tfrecord')
